...
class StockTradingEnvMd(gym.Env):
    ...
    def __init__(
        ...
        self.num_past_days = num_past_days  # num_past_daays + current_day
        self.num_days = num_past_days + 1  # num_past_daays + current_day
        self.day = self.num_days #day
        self.df = df
        ...
        self.observation_space = spaces.Box(low=-10, high=10, shape=(1, self.state_space, self.num_days), dtype=np.float32)
        # self.observation_space = spaces.Box(low=0, high=255, shape=(self.state_space, self.num_days, 1), dtype=np.uint8)

        self.data = self.df.loc[self.day-self.num_past_days:self.day, :]
        # self.data = self.df.loc[self.day-self.num_days:self.day, :]
        self.terminal = False
        ...
        self.asset_memory = [self.calculate_total_asset_value(self.state)]
        self.rewards_memory = []
        self.actions_memory = []
    ...
    def _update_depot(self, actions):
        ...
        if self.turbulence_threshold is not None and self.turbulence > self.turbulence_threshold:
            ...
            for index in sell_index:
                ...
                actions[index], depot, sale_return, cost = self.broker.sell_stock(index, actions[index], self.state[:, -1])
                tmp = self.state[self.buy_price_idxs, -1]
                self.cost += cost
                self.state[self.cash_idx, -1] += sale_return
                self.state[self.depot_idxs, -1] = depot
                if self.state[self.depot_idxs, -1][index] == 0:
                    self.state[self.buy_price_idxs, -1][index] = 0
            ...
            for index in buy_index:
                ...
                actions[index], depot, buy_prices, buy_cost, cost = self.broker.buy_stock(index, actions[index], self.state[:, -1])
                self.state[self.cash_idx, -1] -= buy_cost
                self.state[self.depot_idxs, -1] = depot
                self.state[self.buy_price_idxs, -1] = buy_prices
                self.cost += cost
                # if abs(actions[index]) > 0:
                #     self.trades += 1
    ...
    def calculate_total_asset_value(self, state):
        ...
        if len(state.shape) != 1:
            ...
            current_state = state
        cash = current_state[self.cash_idx]
    ...
    def calculate_total_depot_value(self, state):
        ...
        if len(state.shape) != 1:
            ...
            current_state = state
        prices = np.array(current_state[self.price_idxs])
        depot = np.array(current_state[self.depot_idxs])
    ...
    def calculate_depot_performance(self, state):
        ...
        current_state = state[:, -1]
        prices = np.array(current_state[self.price_idxs])
        buy_prices = np.array(current_state[self.buy_price_idxs])
        depot = np.array(current_state[self.depot_idxs])
    ...
    def calc_portfolio_weight_entropy(self):
        ...
        prices = np.array(self.state[self.price_idxs, -1])
        depot = np.array(self.state[self.depot_idxs, -1])
    ...
    def step(self, actions):
        ...
        if self.terminal:
            ...
            return np.expand_dims(self.state, axis=0), self.reward, self.terminal, info
            ...
            depot_before = self.state[self.depot_idxs, -1]
            buy_prices_before = self.state[self.buy_price_idxs, -1]
            cash_before = self.state[self.cash_idx, -1]
            ...
            depot_after = self.state[self.depot_idxs, -1]
            buy_prices_after = self.state[self.buy_price_idxs, -1]
            cash_after = self.state[self.cash_idx, -1]
            ...
            trade_reward = 0
            for i in sell_index:
                sell_price = self.state[self.price_idxs, -1][i]
                buy_price = buy_prices_before[i]
                amnt_before = depot_before[i]
                weight = abs(actions[i]) / amnt_before
                weight = weight * weight  # square to make it exponential (penalize low amounts stronger)
                ...
                perf_pct = self.calculate_performance(buy_price, sell_price) * 100 * weight
            ...
            if self.turbulence_threshold is not None:
                if len(self.df.tic.unique()) == 1:
                    self.turbulence = self.data[self.risk_indicator_col, -1]
                elif len(self.df.tic.unique()) > 1:
                    self.turbulence = self.data[self.risk_indicator_col, -1].values[0]
            ...
            diversification_reward = -0.5 + weight_entropy * 10
            ...
            no_trade_reward = 0 if self.no_trades_counter < 40 else -2
            # no_trade_reward = 0

            buy_price_low_reward = np.sum(self.state[self.price_idxs, -1] - self.state[self.buy_price_idxs, -1])
            ...
            self.reward = 0
            self.reward += depot_perf_reward * 0.1
            ...
            self.state_memory.append(self.state[:, -1])  # add current state in state_recorder for each step
    ...
    def reset(self):
        ...
        return np.expand_dims(self.state, axis=0)  # Add the image channel dimension for observation
    ...
    def _initiate_state(self):
        if self.initial:
            ...
            cash_hist = [self.initial_amount] * self.num_days
            close_hist = self.data.close.values.tolist()
            depot_hist = [0] * self.stock_dim * self.num_days
            buy_price_hist = [0] * self.stock_dim * self.num_days
            indicator_hist = [self.data[tech].values.tolist() for tech in self.tech_indicator_list]
            indicator_hist = [item for sublist in indicator_hist for item in sublist]  # unpack nested list

            state = cash_hist + close_hist + depot_hist + buy_price_hist + indicator_hist
            state = np.array(state).reshape(self.observation_space.shape[1:])
            ...
            #         + [0] * self.stock_dim                  # Buy prices
            #         + sum(([self.data[tech].values.tolist()] for tech in self.tech_indicator_list), [])  # Not sure what this is
            #     )
        else:
    ...
    def _update_state(self):
        ...
        state_new[self.tech_indicator_start_idx:, -1] = new_indicators

        return state_new

    def _get_date(self):
        if len(self.df.tic.unique()) > 1:
            date = self.data.loc[self.day].date.unique()[0]
        else:
    ...
    def get_sb_env(self, normalize=False):
